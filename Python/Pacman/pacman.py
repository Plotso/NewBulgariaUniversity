...
class Pacman:
    ...
    def draw_grid(self):
        ...
        for x in range(SCREEN_HEIGHT//self.grid_cell_height):
            pygame.draw.line(
                self.background, GREY, (0, x*self.grid_cell_height), (SCREEN_WIDTH, x*self.grid_cell_height))
    ...
    def display_coins_and_walls_paths(self):
        ...
        if SHOULD_DISPLAY_COLORED_WALLS:
            for wall in self.walls:
                pygame.draw.rect(self.background, (112, 55, 163), (wall.x*self.grid_cell_width, wall.y*self.grid_cell_height, self.grid_cell_width, self.grid_cell_height))

        if SHOULD_DISPLAY_COLORED_COINS_PATH:
            for coin in self.coins:
                pygame.draw.rect(self.background, (167, 179, 34), (
                coin.x * self.grid_cell_width, coin.y * self.grid_cell_height, self.grid_cell_width, self.grid_cell_height))
    ...
    def game_on_draw(self):
        ...
        self.draw_text('CURRENT SCORE: {}'.format(self.player.current_score), self.screen, current_score_position, 18, WHITE, DEFAULT_FONT)
        self.draw_text('HIGH SCORE: {}'.format(self.high_score), self.screen, hight_score_position, 18, WHITE, DEFAULT_FONT)
    ...
    def draw_coins(self):
        ...
        for coin in self.coins:
            radius = 5
            pygame.draw.circle(self.screen, COINS_COLOUR,
                               (int(coin.x*self.grid_cell_width)+self.grid_cell_width//2+SCREEN_BUFFER//2,
                                int(coin.y*self.grid_cell_height)+self.grid_cell_height//2+SCREEN_BUFFER//2), radius)
